    ...
    [update: "/update"]: respond_to {
        ...
        POST: json_params =>
            if const_compare @params.token, slack_tokens
                ...
                --unless message
                var = os.execute "curl -X POST --data-urlencode 'payload={\"channel\": \"#slackiver\", \"username\": \"The Slackiver\", \"text\": \"Error occured saving message from #{@params.user_name}: #{@params.text} (Sent at #{@params.timestamp} in #{@params.channel_name}.)\", \"icon_emoji\": \":warning:\"}' #{slack_hook}"
